...
def _get_cell_indexes(
    x: Series[float], y: Series[float], a: int = N, w: int = M
) -> tuple[Series[int], Series[int]]:
    xi = x.divide(spadlconfig.field_length).multiply(a)
    yj = y.divide(spadlconfig.field_width).multiply(w)
    xi = xi.astype("int64").clip(0, a - 1)
    yj = yj.astype("int64").clip(0, w - 1)
    return xi, yj


def _get_flat_indexes(
    x: Series[float], y: Series[float], a: int = N, w: int = M
) -> Series[int]:
    xi, yj = _get_cell_indexes(x, y, a, w)
    return yj.rsub(w - 1).mul(a).add(xi)
...
def _count(
    ...
    flat_indexes = _get_flat_indexes(x, y, a, w)
    vc = flat_indexes.value_counts(sort=False)
    vector = np.zeros(w * a, dtype=int)
    vector[vc.index] = vc
    return vector.reshape((w, a))


def _safe_divide(a: npt.ArrayLike, b: npt.ArrayLike) -> npt.NDArray[np.float64]:
    return np.divide(
        a, b, out=np.zeros_like(a, dtype="float64"), where=b != 0, casting="unsafe"
    )


def scoring_prob(
    actions: DataFrame[SPADLSchema], a: int = N, w: int = M
    ...
    goals = shot_actions[
        (shot_actions.result_id == spadlconfig.results.index("success"))
    ]

    shotmatrix = _count(shot_actions.start_x, shot_actions.start_y, a, w)
    goalmatrix = _count(goals.start_x, goals.start_y, a, w)
...
def get_successful_move_actions(
    ...
    return move_actions[
        (move_actions.result_id == spadlconfig.results.index("success"))
    ]
...
def move_transition_matrix(
    ...
    for i in range(0, w * l):
        vc2 = X[
            (
                (X.start_cell == i)
                & (X.result_id == spadlconfig.results.index("success"))
            )
...
class ExpectedThreat:
    ...
    def rate(
        ...
        startxc, startyc = _get_cell_indexes(actions.x_A, actions.y_A, l, w)
        endxc, endyc = _get_cell_indexes(
            actions.pass_end_x_B, actions.pass_end_y_B, l, w
        )

        xT_start = grid[startyc.rsub(w - 1), startxc]
        xT_end = grid[endyc.rsub(w - 1), endxc]

        ratings[actions.index] = xT_end - xT_start
        return ratings
...
def load_model(path: str) -> ExpectedThreat:
    """Create a model from a pre-computed xT value surface (SSL verification disabled for URLs)."""

    # Detect if it's a URL
    if path.startswith(("http://", "https://")):
        response = requests.get(path, verify=False)  # Disable SSL verification
        response.raise_for_status()
        grid = pd.read_json(StringIO(response.text))
    else:
        # Local file path
